...
| `RET` | `0x0F` | 1 | неявная | 3 | `--` | Вернуться из процедуры |
| `CALLXT` | `0x10` | 1 | неявная | 2 | `xt --` | Вызвать процедуру по `execution token` |
| `TOR` | `0x11` | 1 | неявная | 2 | `a --` | Перенести значение на стек возвратов |
| `FROMR` | `0x12` | 1 | неявная | 3 | `-- a` | Перенести значение со стека возвратов |
| `RPEEK` | `0x13` | 1 | неявная | 3 | `-- a` | Скопировать вершину стека возвратов |
| `EI` | `0x14` | 1 | неявная | 2 | `--` | Установить `IE = 1` в `flags` |
| `DI` | `0x15` | 1 | неявная | 2 | `--` | Установить `IE = 0` в `flags` |
| `IRET` | `0x16` | 1 | неявная | 3 | `--` | Вернуться из обработчика прерывания |
| `GET_CARRY` | `0x17` | 1 | неявная | 2 | `-- flag` | Положить на стек флаг переноса `C` |
| `LOADA addr` | `0x18` | 3 | абсолютная | 4 | `-- value` | Прочитать слово data memory по адресу из команды |
| `STOREA addr` | `0x19` | 3 | абсолютная | 3 | `value --` | Записать слово data memory по адресу из команды |
| `ADDM addr` | `0x1A` | 3 | абсолютная | 4 | `a -- result` | Сложить `tos` с `DataMem[addr]` |
| `JMP rel` | `0x1B` | 3 | относительная | 3 | `--` | Безусловный переход |
| `JZ rel` | `0x1C` | 3 | относительная | 3 | `flag --` | Переход, если `flag = 0` |
| `CALL rel` | `0x1D` | 3 | относительная | 3 | `--` | Вызвать процедуру по относительному смещению |
| `PUSHI32 imm` | `0x1E` | 5 | непосредственная | 3 | `-- x` | Положить 32-битный литерал |
| `PUSHN k v0 ... v{k-1}` | `0x1F` | `2+4k` | непосредственная | `3+2k` | `-- v0 ... v{k-1}` | Положить на стек `k` 32-битных значений |
