...
- `MEM_READ` -- чтение из data memory, MMIO или return stack в регистр `read_data`;
- `WRITEBACK` -- запись ранее прочитанного `read_data` в стек или регистры;
...
- `EXECUTE_PUSHN_VALUE` -- запись уже защелкнутого значения `arg` на стек данных;
...
Такты указаны для модели `machine.py`. Однобайтовые команды без явного операнда обычно выполняются за `FETCH_OPCODE + EXECUTE`. Команды, которым нужно прочитать data memory, MMIO или return stack, используют отдельный такт `MEM_READ` и следующий такт для использования `read_data`. Для `PUSHN` значение `k` равно количеству 32-битных значений в команде: каждое значение сначала защелкивается в `arg`, а следующим тактом записывается на стек.

| Инструкция | Opcode | Длина | Адресация | Такты | Стек | Семантика |
| --- | ---: | ---: | --- | ---: | --- | --- |
| `HALT` | `0x00` | 1 | неявная | 2 | `--` | Остановить модель |
| `DUP` | `0x01` | 1 | неявная | 2 | `a -- a a` | Дублировать вершину стека |
| `DROP` | `0x02` | 1 | неявная | 2 | `a --` | Удалить вершину стека |
| `SWAP` | `0x03` | 1 | неявная | 2 | `a b -- b a` | Обменять два верхних значения |
| `OVER` | `0x04` | 1 | неявная | 2 | `a b -- a b a` | Скопировать второе значение |
| `LOAD` | `0x05` | 1 | косвенная | 3 | `addr -- value` | Прочитать слово data memory по адресу со стека |
| `STORE` | `0x06` | 1 | косвенная | 2 | `value addr --` | Записать слово data memory по адресу со стека |
| `ADD` | `0x07` | 1 | неявная | 2 | `a b -- a+b` | Сложение |
| `SUB` | `0x08` | 1 | неявная | 2 | `a b -- a-b` | Вычитание |
| `MUL` | `0x09` | 1 | неявная | 2 | `a b -- a*b` | Умножение |
| `DIV` | `0x0A` | 1 | неявная | 2 | `a b -- a/b` | Целочисленное деление |
| `MOD` | `0x0B` | 1 | неявная | 2 | `a b -- a%b` | Остаток от деления |
| `EQ` | `0x0C` | 1 | неявная | 2 | `a b -- flag` | Проверить равенство |
| `LT` | `0x0D` | 1 | неявная | 2 | `a b -- flag` | Проверить `a < b` |
| `GT` | `0x0E` | 1 | неявная | 2 | `a b -- flag` | Проверить `a > b` |
| `AND` | `0x0F` | 1 | неявная | 2 | `a b -- flag` | Логическое И |
| `OR` | `0x10` | 1 | неявная | 2 | `a b -- flag` | Логическое ИЛИ |
| `NOT` | `0x11` | 1 | неявная | 2 | `a -- flag` | Логическое НЕ |
| `RET` | `0x12` | 1 | неявная | 3 | `--` | Вернуться из процедуры |
| `CALLXT` | `0x13` | 1 | неявная | 2 | `xt --` | Вызвать процедуру по `execution token` |
| `TOR` | `0x14` | 1 | неявная | 2 | `a --` | Перенести значение на стек возвратов |
| `FROMR` | `0x15` | 1 | неявная | 3 | `-- a` | Перенести значение со стека возвратов |
| `RPEEK` | `0x16` | 1 | неявная | 3 | `-- a` | Скопировать вершину стека возвратов |
| `EI` | `0x17` | 1 | неявная | 2 | `--` | Установить `IE = 1` в `flags` |
| `DI` | `0x18` | 1 | неявная | 2 | `--` | Установить `IE = 0` в `flags` |
| `IRET` | `0x19` | 1 | неявная | 3 | `--` | Вернуться из обработчика прерывания |
| `GET_CARRY` | `0x1A` | 1 | неявная | 2 | `-- flag` | Положить на стек флаг переноса `C` |
| `LOADA addr` | `0x1B` | 3 | абсолютная | 4 | `-- value` | Прочитать слово data memory по адресу из команды |
| `STOREA addr` | `0x1C` | 3 | абсолютная | 3 | `value --` | Записать слово data memory по адресу из команды |
| `ADDM addr` | `0x1D` | 3 | абсолютная | 4 | `a -- result` | Сложить `tos` с `DataMem[addr]` |
| `MULM addr` | `0x1E` | 3 | абсолютная | 4 | `a -- result` | Умножить `tos` на `DataMem[addr]` |
| `JMP rel` | `0x1F` | 3 | относительная | 3 | `--` | Безусловный переход |
| `JZ rel` | `0x20` | 3 | относительная | 3 | `flag --` | Переход, если `flag = 0` |
| `CALL rel` | `0x21` | 3 | относительная | 3 | `--` | Вызвать процедуру по относительному смещению |
| `PUSHI32 imm` | `0x22` | 5 | непосредственная | 3 | `-- x` | Положить 32-битный литерал |
| `PUSHN k v0 ... v{k-1}` | `0x23` | `2+4k` | непосредственная | `3+2k` | `-- v0 ... v{k-1}` | Положить на стек `k` 32-битных значений |
